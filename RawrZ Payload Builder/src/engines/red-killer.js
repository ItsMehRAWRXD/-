...
class RedKiller {
    ...
    async testRegistryAccess() {
        return new Promise((resolve) => {
            ...
            // Try to read a system registry key with timeout
            const timeout = setTimeout(() => {
                logger.warn('[Red Killer] Registry access test timed out - likely blocked by security software');
                resolve(false);
            }, 3000);
            ...
            exec('reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion" /v ProductName', (error) => {
                ...
                if (error) {
                    logger.warn(`[Red Killer] Registry access blocked: ${error.message}`);
                }
                resolve(!error);
            });
        });
