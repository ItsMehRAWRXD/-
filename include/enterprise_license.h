...
struct FeatureMask
{
    ...
    {
        if (bit < 64)
            return (lo & (1ULL << bit)) != 0;
        else
            return (hi & (1ULL << (bit - 64))) != 0;
    }
    ...
    constexpr FeatureMask operator|(const FeatureMask& o) const { return {lo | o.lo, hi | o.hi}; }

    constexpr FeatureMask operator&(const FeatureMask& o) const { return {lo & o.lo, hi & o.hi}; }

    constexpr bool operator==(const FeatureMask& o) const { return lo == o.lo && hi == o.hi; }
...
inline FeatureMask forTier(LicenseTierV2 tier)
{
    switch (tier)
    {
        case LicenseTierV2::Community:
            return Community();
        case LicenseTierV2::Professional:
            return Professional();
        case LicenseTierV2::Enterprise:
            return Enterprise();
        case LicenseTierV2::Sovereign:
            return Sovereign();
        default:
            return Community();
    }
...
struct LicenseResult
{
    ...
    static LicenseResult ok(const char* msg = "OK") { return {true, msg, 0}; }
    static LicenseResult error(const char* msg, int code = -1) { return {false, msg, code}; }
};
...
class EnterpriseLicenseV2
{
  public:
    ...
    LicenseResult createKey(LicenseTierV2 tier, uint32_t durationDays, const char* signingSecret,
...
#define LICENSE_GATE(feature)                                                                                          \
    if (!RawrXD::License::EnterpriseLicenseV2::Instance().gate(RawrXD::License::FeatureID::feature, __FUNCTION__))     \
    return
