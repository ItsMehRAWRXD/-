...
namespace
{
// routeCommandUnified sets ctx.hwnd to the main frame; ctx.idePtr is Win32IDE* (never HWND*).
static HWND resolveGuiMainHwnd(const CommandContext& ctx)
{
    ...
    return ide ? ide->getMainWindow() : nullptr;
}
}  // namespace
...
CommandResult handleFileNew(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        ...
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 1001, 0);  // file.new (COMMAND_TABLE)
    ...
    else
    {
        ...
        HANDLE h = CreateFileA(tmpFile, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
...
CommandResult handleFileOpen(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 1002, 0);  // file.open
    ...
    else
    {
        ...
        HANDLE h = CreateFileA(ctx.args, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL,
                               nullptr);
        if (h == INVALID_HANDLE_VALUE)
        {
...
CommandResult handleFileSave(const CommandContext& ctx)
{
    ...
    else
    {
        ...
        HANDLE h = CreateFileA(ctx.args, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
        if (h == INVALID_HANDLE_VALUE)
        {
            std::string msg =
                "[File] Cannot write: " + std::string(ctx.args) + " (err " + std::to_string(GetLastError()) + ")\n";
            ctx.output(msg.c_str());
...
CommandResult handleFileSaveAs(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 1004, 0);  // file.saveAs
    ...
    else
    {
        ...
        {
            std::string msg =
                "[File] Cannot create: " + std::string(ctx.args) + " (err " + std::to_string(GetLastError()) + ")\n";
            ctx.output(msg.c_str());
...
CommandResult handleFileSaveAll(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 1005, 0);  // file.saveAll
...
CommandResult handleFileClose(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 1006, 0);  // file.close
...
CommandResult handleFileModelFromHF(const CommandContext& ctx)
{
    ...
    HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", wPath.c_str(), nullptr, WINHTTP_NO_REFERER,
...
CommandResult handleEditUndo(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2007, 0);  // IDM_EDIT_UNDO → Win32IDE::handleEditCommand
...
CommandResult handleEditRedo(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2008, 0);  // IDM_EDIT_REDO (was 2002 = file.open — fixed)
...
CommandResult handleEditCut(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2009, 0);  // IDM_EDIT_CUT
...
CommandResult handleEditCopy(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2010, 0);  // IDM_EDIT_COPY
...
CommandResult handleEditPaste(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2011, 0);  // IDM_EDIT_PASTE
...
CommandResult handleEditSelectAll(const CommandContext& ctx)
{
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
            PostMessageA(hwnd, WM_COMMAND, 2006, 0);  // IDM_EDIT_SELECT_ALL
...
CommandResult handleAgentBoundedLoop(const CommandContext& ctx)
{
    ...
    auto stats = orchestrator.getStats();
    std::ostringstream oss;
    oss << "[Agent] Poll complete. Anomalies: " << stats.anomaliesDetected << ", Repairs: " << stats.repairsAttempted
        << "\n";
    ctx.output(oss.str().c_str());
...
CommandResult handleAgentViewTools(const CommandContext& ctx)
{
    ...
    oss << "=== Available Agent Tools (with status) ===\n"
        ...
        << "  6. ProxyHotpatcher         biases=" << proxyStats.biasesApplied.load()
        << " rewrites=" << proxyStats.rewritesApplied.load() << "\n"
...
CommandResult handleAgentViewStatus(const CommandContext& ctx)
{
    ...
    oss << "=== Agent System Status ===\n"
        ...
        << "  Sentinel:     " << (sentinel.isActive() ? "ACTIVE" : "inactive") << " (" << sentinel.getViolationCount()
        << " violations)\n"
...
CommandResult handleAutonomyRate(const CommandContext& ctx)
{
    ...
    auto stats = orchestrator.getStats();
    double rate =
        (stats.totalPollCycles > 0) ? (double)stats.repairsAttempted / (double)stats.totalPollCycles * 100.0 : 0.0;
    std::ostringstream oss;
    oss << "[Autonomy] Repair rate: " << rate << "% (" << stats.repairsAttempted << "/" << stats.totalPollCycles
        << " polls)\n";
    ctx.output(oss.str().c_str());
...
CommandResult handleSubAgentSwarm(const CommandContext& ctx)
{
    ...
    {
        oss << "  [" << (i + 1) << "] " << caps[i].switchName << (caps[i].selfContained ? " (self-contained)" : "")
...
CommandResult handleSubAgentStatus(const CommandContext& ctx)
{
    ...
    oss << "=== Sub-Agent Status ===\n"
        ...
        << "  Todo items:        " << g_subAgentState.todos.size() << " (" << pendingTodos << " pending)\n"
...
static CommandResult launchTerminal(const CommandContext& ctx, const char* tag)
{
    ...
    BOOL ok = CreateProcessA(nullptr, cmdLine, nullptr, nullptr, FALSE, CREATE_NEW_CONSOLE, nullptr, nullptr, &si, &pi);
    if (!ok)
    {
        char buf[128];
...
CommandResult handleTerminalSplitH(const CommandContext& ctx)
{
    if (ctx.args && ctx.args[0])
    {
        ...
        if (_stricmp(arg, "v") == 0 || _stricmp(arg, "vert") == 0 || _stricmp(arg, "vertical") == 0)
        {
...
CommandResult handleHotpatchStatus(const CommandContext& ctx)
{
    ...
    oss << "=== Hotpatch System Status ===\n"
        << "  Memory Layer:  " << memStats.totalApplied.load() << " applied, " << memStats.totalReverted.load()
        << " reverted, " << memStats.totalFailed.load() << " failed\n"
        << "  Byte Layer:    active\n"
        << "  Server Layer:  active\n"
        << "  Proxy Layer:   " << proxyStats.biasesApplied.load() << " biases, " << proxyStats.rewritesApplied.load()
        << " rewrites, " << proxyStats.validationsPassed.load() << " validations\n"
        << "  Unified Total: " << stats.totalOperations.load() << " applied, " << stats.totalFailures.load()
        << " failures\n"
        << "  Sentinel:      " << (SentinelWatchdog::instance().isActive() ? "ACTIVE" : "inactive") << "\n";
    ...
    oss << "  Context Deterioration: " << (ctxHotpatch.isEnabled() ? "ON (100% quality)" : "OFF")
        ...
        << " mitigations=" << ctxStats.mitigationsApplied.load() << " tokens_saved=" << ctxStats.tokensSaved.load()
        << "\n";
    ctx.output(oss.str().c_str());
...
CommandResult handleHotpatchByte(const CommandContext& ctx)
{
    ...
    HANDLE hFile = CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                               FILE_ATTRIBUTE_NORMAL, nullptr);
    if (hFile == INVALID_HANDLE_VALUE)
    {
        ctx.output(
            ("[BytePatch] Cannot open: " + filename + " (err " + std::to_string(GetLastError()) + ")\n").c_str());
...
static struct AIModeState
{
    ...
    static constexpr ModeConfig modes[] = {{"chat", 4096, false, false, false},
...
CommandResult handleAIModeSet(const CommandContext& ctx)
{
    if (!ctx.args || !ctx.args[0])
    {
        ...
        {
            oss << "  " << m.name << " (tokens=" << m.maxTokens << ", agent=" << (m.enableAgent ? "yes" : "no")
                << ", research=" << (m.enableResearch ? "yes" : "no") << ")";
            if (g_aiMode.currentMode == m.name)
                oss << " <<ACTIVE>>";
            oss << "\n";
        }
        ctx.output(oss.str().c_str());
...
CommandResult handleAIEngineSelect(const CommandContext& ctx)
{
    if (!ctx.args || !ctx.args[0])
    {
        ...
        for (size_t i = 0; i < g_backendCfg.backends.size(); i++)
        {
            ...
            oss << "  [" << (i + 1) << "] " << b.name << (b.available ? " [available]" : " [not tested]");
...
extern "C" long long RunInference_fallback(const char* prompt, long long maxTokens, char* output)
{
    ...
    if (prompt && prompt[0])
    {
        ...
        if (client.TestConnection())
        {
            ...
            msgs.push_back(
                {"system",
                 "You are in deep-thinking mode. Return concise, reasoned output and terminate with </thought>."});
...
extern "C" int rawrxd_agentic_deep_think_loop_fallback(const char* prompt)
{
    ...
    for (int step = 0; step < 4; ++step)
    {
        ...
        aggregate.append("\n");
        if (aggregate.find("</thought>") != std::string::npos)
            break;
...
extern "C" __int64 OverclockGov_ApplyOffset_fallback(uint32_t domain, int32_t offsetMhz)
{
    ...
    {
        case 0:
            return OverclockGov_ApplyCpuOffset_fallback(offsetMhz);
        case 1:
            return OverclockGov_ApplyGpuOffset_fallback(offsetMhz);
        case 2:
            return OverclockGov_ApplyMemoryOffset_fallback(offsetMhz);
        case 3:
            return OverclockGov_ApplyStorageOffset_fallback(offsetMhz);
        default:
            return -3;
    }
}
...
{
    std::lock_guard<std::mutex> lock(g_overclockFallbackState.mtx);
    if (domain == 1 && g_overclockFallbackState.gpuBackendReady)
        return 58 + (g_overclockFallbackState.gpuOffsetMhz / 25);
    if (domain == 0)
        return 62 + (g_overclockFallbackState.cpuOffsetMhz / 30);
    return 45;
}
...
extern "C" __int64 OverclockGov_ReadFrequency_fallback(uint32_t domain)
{
    std::lock_guard<std::mutex> lock(g_overclockFallbackState.mtx);
    if (domain == 1)
        return 1800 + g_overclockFallbackState.gpuOffsetMhz;
    if (domain == 0)
        return 4200 + g_overclockFallbackState.cpuOffsetMhz;
    if (domain == 2)
        return 3200 + g_overclockFallbackState.memoryOffsetMhz;
    return 0;
}
...
extern "C" __int64 OverclockGov_ReadPowerDraw_fallback(uint32_t domain)
{
    std::lock_guard<std::mutex> lock(g_overclockFallbackState.mtx);
    if (domain == 1)
        return 140 + (g_overclockFallbackState.gpuOffsetMhz / 8);
    if (domain == 0)
        return 95 + (g_overclockFallbackState.cpuOffsetMhz / 10);
    return 25;
}
...
extern "C" __int64 OverclockGov_ReadUtilization_fallback(uint32_t domain)
{
    std::lock_guard<std::mutex> lock(g_overclockFallbackState.mtx);
    if (domain == 1)
        return g_overclockFallbackState.gpuBackendReady ? 72 : 0;
    if (domain == 0)
        return 55;
    return 20;
}
...
CommandResult handleREDecisionTree(const CommandContext& ctx)
{
    ...
    while ((pos = output.find("j", pos)) != std::string::npos)
    {
        if (pos + 1 < output.size() &&
            (output[pos + 1] == 'e' || output[pos + 1] == 'n' || output[pos + 1] == 'z' || output[pos + 1] == 'a' ||
             output[pos + 1] == 'b' || output[pos + 1] == 'l' || output[pos + 1] == 'g'))
        {
            branches++;
        }
        pos++;
...
CommandResult handleREAutoPatch(const CommandContext& ctx)
{
    ...
    bool isValid = headers.find("PE signature") != std::string::npos ||
                   headers.find("machine (x64)") != std::string::npos || headers.find("magic") != std::string::npos;
    const auto& stats = mgr.getStats();
    std::ostringstream oss;
...
CommandResult handleREDisassemble(const CommandContext& ctx)
{
    ...
    if (lineCount >= 80)
        ctx.output("  ... (truncated, use dumpbin directly for full output)\n");
...
CommandResult handleConfidence(const CommandContext& ctx)
{
    ...
    if (it2 != stats.failureTypeCounts.end())
        halluCount = it2->second;
    ...
    if (it2 != stats.failureTypeCounts.end())
        timeoutCount = it2->second;
    double confidence = (stats.totalOutputsAnalyzed > 0)
                            ? (1.0 - (double)halluCount / (double)stats.totalOutputsAnalyzed) * 100.0
                            : 100.0;
    std::ostringstream oss;
...
CommandResult handleServerStart(const CommandContext& ctx)
{
    ...
    si.cb = sizeof(si);
    PROCESS_INFORMATION pi = {};
    BOOL ok = CreateProcessA(nullptr, cmdLine, nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr, nullptr, &si, &pi);
    if (!ok)
    {
        std::ostringstream oss;
...
CommandResult handleThemeSet(const CommandContext& ctx)
{
    if (!ctx.args || !ctx.args[0])
    {
        ...
        {
            oss << "  " << t.name;
            if (strcmp(t.name, g_activeTheme) == 0)
                oss << " <<ACTIVE>>";
            oss << "\n";
        }
        ctx.output(oss.str().c_str());
...
CommandResult handleBackendList(const CommandContext& ctx)
{
    ...
    for (size_t i = 0; i < g_backendCfg.backends.size(); i++)
    {
        ...
        oss << "  [" << (i + 1) << "] " << b.name
            ...
            << b.endpoint << (b.available ? " [available]" : " [not tested]") << "\n";
    }
    ctx.output(oss.str().c_str());
...
CommandResult handleSwarmJoin(const CommandContext& ctx)
{
    ...
    if (ctx.args && ctx.args[0])
    {
        ...
        std::ostringstream oss;
        oss << "[Swarm] Joined cluster via " << node.address << ". Total nodes: " << g_swarmState.nodes.size() << "\n";
        ctx.output(oss.str().c_str());
    }
    else
    {
...
CommandResult handleSwarmDistribute(const CommandContext& ctx)
{
    ...
    for (auto& n : g_swarmState.nodes)
    {
        if (n.connected)
        {
            ...
            distributed = true;
            std::ostringstream oss;
            oss << "[Swarm] Task #" << g_swarmState.totalTasks << " distributed to " << n.name << " (" << n.address
                << ")\n";
            ctx.output(oss.str().c_str());
            break;
        }
...
CommandResult handleSwarmNodes(const CommandContext& ctx)
{
    ...
    for (size_t i = 0; i < g_swarmState.nodes.size(); i++)
    {
        ...
        oss << "  [" << i << "] " << n.name << " @ " << n.address << (n.connected ? " [CONNECTED]" : " [offline]")
            << " tasks=" << n.taskCount << "\n";
    }
    ctx.output(oss.str().c_str());
...
{
#ifdef _WIN32
    // GUI: same path as Help ▸ About (IDM_HELP_ABOUT = 7904 → handleHelpCommand → About box).
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
        {
            PostMessageA(hwnd, WM_COMMAND, static_cast<WPARAM>(7904), 0);
...
{
#ifdef _WIN32
    // GUI: same dialog as Help palette "Keyboard Shortcuts" (7905 → handleHelpCommand case 7005).
    if (ctx.isGui && ctx.idePtr)
    {
        HWND hwnd = resolveGuiMainHwnd(ctx);
        if (hwnd)
        {
            PostMessageA(hwnd, WM_COMMAND, static_cast<WPARAM>(7905), 0);
...
CommandResult handleSearch(const CommandContext& ctx)
{
    ...
    if (count > 0)
    {
        ctx.output(oss.str().c_str());
        if (count >= 50)
            ctx.output("  ... (truncated at 50 results)\n");
    }
    else
    {
...
CommandResult handleAnalyze(const CommandContext& ctx)
{
    ...
    while (fgets(buf, sizeof(buf), f))
    {
        ...
        if (strstr(buf, "CommandResult ") || strstr(buf, "void ") || strstr(buf, "int "))
            funcs++;
        if (strstr(buf, "class ") || strstr(buf, "struct "))
            classes++;
    }
    fclose(f);
    std::ostringstream oss;
...
CommandResult handleCOT(const CommandContext& ctx)
{
    ...
    if (cotEnabled)
    {
        ...
        oss << "[COT] Chain-of-thought mode: ENABLED\n"
            ...
            << "  - Step markers active\n"
            << "  - Proxy stats: " << stats.biasesApplied.load() << " biases, " << stats.rewritesApplied.load()
            << " rewrites\n";
        ctx.output(oss.str().c_str());
    }
    else
...
{
    auto& reg = SharedFeatureRegistry::instance();
    std::string msg = "[status] " + std::to_string(reg.totalRegistered()) + " features registered, " +
                      std::to_string(reg.totalDispatched()) + " dispatches\n";
    ctx.output(msg.c_str());
...
CommandResult handleHelp(const CommandContext& ctx)
{
    ...
    for (const auto* f : features)
    {
        ...
        {
            std::string line = "  " + std::string(f->cliCommand) + " — " + std::string(f->name) + " (" +
