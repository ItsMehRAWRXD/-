...
struct EditorTab
{
    ...
    // Persistence fields for TabManager
    int cursorLine = 1;
    int cursorCol = 0;
    int scrollPos = 0;
    std::vector<std::pair<int, int>> multiCursors;  // additional cursor positions (line, col)
    std::set<std::pair<int, int>> foldedRegions;    // folded line ranges (start, end)
};
...
struct GitStatus
{
    ...
    std::map<std::string, char> fileStatus;  // file path -> status char (M/A/D/?)
};
