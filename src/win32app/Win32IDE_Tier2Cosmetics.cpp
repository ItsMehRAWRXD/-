...
void Win32IDE::initGitDiffViewer()
{
    ...
    m_gitDiffFont =
        CreateFontA(-dpiScale(13), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
...
void Win32IDE::shutdownGitDiffViewer()
{
    ...
    if (m_gitDiffFont)
    {
        DeleteObject(m_gitDiffFont);
        m_gitDiffFont = nullptr;
    }
}
...
void Win32IDE::showGitDiffSideBySide(const std::string& filePath)
{
    ...
    m_gitDiffLeftLines.clear();
    {
        ...
        if (CreatePipe(&hR, &hW, &sa, 0))
        {
            ...
            if (CreateProcessA(NULL, cmdBuf2, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si2, &pi2))
            {
                CloseHandle(hW);
                hW = NULL;
    ...
    LOG_INFO("[GitDiff] Showing diff for: " + filePath + " (" + std::to_string(m_gitDiffHunks.size()) + " hunks)");
}
...
void Win32IDE::createGitDiffPanel()
{
    ...
    m_hwndGitDiffPanel = CreateWindowExA(WS_EX_CLIENTEDGE, GITDIFF_CLASS, "", WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN,
                                         0, dpiScale(35), panelW, panelH - dpiScale(60), m_hwndMain,
                                         (HMENU)(UINT_PTR)IDC_GITDIFF_PANEL, GetModuleHandle(NULL), NULL);
    ...
    m_hwndGitDiffLeft = CreateWindowExA(
        0, "RichEdit20A", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL, 0,
    ...
    m_hwndGitDiffRight = CreateWindowExA(
        0, "RichEdit20A", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL, halfW,
    ...
    CreateWindowExA(0, "BUTTON", "< Prev Hunk", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, halfW - dpiScale(160),
                    dpiScale(4), dpiScale(75), dpiScale(24), m_hwndGitDiffPanel, (HMENU)(UINT_PTR)IDC_GITDIFF_PREV,
                    GetModuleHandle(NULL), NULL);

    CreateWindowExA(0, "BUTTON", "Next Hunk >", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, halfW - dpiScale(80),
                    dpiScale(4), dpiScale(75), dpiScale(24), m_hwndGitDiffPanel, (HMENU)(UINT_PTR)IDC_GITDIFF_NEXT,
                    GetModuleHandle(NULL), NULL);

    CreateWindowExA(0, "BUTTON", "Close", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, panelW - dpiScale(70), dpiScale(4),
                    dpiScale(60), dpiScale(24), m_hwndGitDiffPanel, (HMENU)(UINT_PTR)IDC_GITDIFF_CLOSE,
                    GetModuleHandle(NULL), NULL);
...
void Win32IDE::populateGitDiffPane(HWND hwndRichEdit, const std::vector<DiffLine>& lines, const std::string& header)
{
    if (!hwndRichEdit)
        return;
...
void Win32IDE::navigateDiffHunk(int direction)
{
    ...
    if (m_gitDiffCurrentHunk < 0)
        m_gitDiffCurrentHunk = (int)m_gitDiffHunks.size() - 1;
    if (m_gitDiffCurrentHunk >= (int)m_gitDiffHunks.size())
        m_gitDiffCurrentHunk = 0;
    ...
    switch (msg)
    {
        ...
        case WM_COMMAND:
            if (ide)
            {
                ...
                if (id == IDC_GITDIFF_CLOSE)
                {
                    ide->closeGitDiffViewer();
                    return 0;
                }
                if (id == IDC_GITDIFF_PREV)
                {
                    ide->navigateDiffHunk(-1);
                    return 0;
                }
                if (id == IDC_GITDIFF_NEXT)
                {
                    ide->navigateDiffHunk(1);
                    return 0;
                }
...
void Win32IDE::initTerminalTabs()
{
    ...
    LOG_INFO("[TerminalTabs] Initialized with " + std::to_string(m_terminalTabProfiles.size()) + " profiles");
}
...
void Win32IDE::shutdownTerminalTabs()
{
    ...
    m_terminalTabCwdTelemetryCarry.clear();
    m_terminalTabIntegratedWorkingDirectory.clear();
    m_terminalTabGitBranchFromCwd.clear();
    m_terminalTabProfiles.clear();

    if (m_hwndTerminalTabBar && IsWindow(m_hwndTerminalTabBar))
    {
        RemovePropA(m_hwndTerminalTabBar, "IDE_PTR");
        DestroyWindow(m_hwndTerminalTabBar);
    }
    m_hwndTerminalTabBar = nullptr;
    m_activeTerminalTab = 0;
    LOG_INFO("[TerminalTabs] Shutdown complete");
}

void Win32IDE::createTerminalTabBar()
{
    if (m_hwndTerminalTabBar || !m_hwndPowerShellPanel)
        return;
    ...
    m_hwndTerminalTabBar =
        CreateWindowExA(0, "STATIC", "", WS_CHILD | WS_VISIBLE | SS_OWNERDRAW, 0, 0, panelRect.right, dpiScale(28),
                        m_hwndPowerShellPanel, (HMENU)(UINT_PTR)IDC_TERMTAB_BAR, GetModuleHandle(NULL), NULL);
    ...
    CreateWindowExA(0, "BUTTON", "+", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, panelRect.right - dpiScale(60),
                    dpiScale(2), dpiScale(24), dpiScale(22), m_hwndPowerShellPanel, (HMENU)(UINT_PTR)IDC_TERMTAB_ADD,
                    GetModuleHandle(NULL), NULL);

    // Profile dropdown
    CreateWindowExA(0, "BUTTON", "v", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, panelRect.right - dpiScale(32),
                    dpiScale(2), dpiScale(24), dpiScale(22), m_hwndPowerShellPanel,
                    (HMENU)(UINT_PTR)IDC_TERMTAB_DROPDOWN, GetModuleHandle(NULL), NULL);
...
{
    if (profileIndex < 0 || profileIndex >= (int)m_terminalTabProfiles.size())
        return;
    ...
    std::string cwdStore;
    const char* cwdPtr = preferredIntegratedTerminalWorkingDirectory(cwdStore);

    m_terminalTabs.push_back(std::move(tab));
    const size_t tabIndex = m_terminalTabs.size() - 1;
    m_terminalTabCwdTelemetryCarry.resize(m_terminalTabs.size());
    m_terminalTabIntegratedWorkingDirectory.resize(m_terminalTabs.size());
    m_terminalTabGitBranchFromCwd.resize(m_terminalTabs.size());
    if (!cwdStore.empty())
        m_terminalTabIntegratedWorkingDirectory[tabIndex] = cwdStore;
    else
    {
        char buf[MAX_PATH] = {};
        if (GetCurrentDirectoryA(MAX_PATH, buf))
            m_terminalTabIntegratedWorkingDirectory[tabIndex] = buf;
    }
    {
        std::string br0;
        queryGitBranchForIntegratedCwd(m_terminalTabIntegratedWorkingDirectory[tabIndex], br0);
        m_terminalTabGitBranchFromCwd[tabIndex] = std::move(br0);
    }

    Win32TerminalManager* mgr = m_terminalTabs[tabIndex].manager.get();
    mgr->onOutput = [this, tabIndex](const std::string& output)
    {
        if (isShuttingDown())
            return;
        if (tabIndex >= m_terminalTabCwdTelemetryCarry.size())
            return;
        std::string cwdUpd;
        std::string& carry = m_terminalTabCwdTelemetryCarry[tabIndex];
        const std::string filtered = filterTerminalCwdTelemetry(carry, output, &cwdUpd);
        if (!cwdUpd.empty() && tabIndex < m_terminalTabIntegratedWorkingDirectory.size())
        {
            const bool cwdChanged = (m_terminalTabIntegratedWorkingDirectory[tabIndex] != cwdUpd);
            m_terminalTabIntegratedWorkingDirectory[tabIndex] = std::move(cwdUpd);
            if (cwdChanged && tabIndex < m_terminalTabGitBranchFromCwd.size())
            {
                std::string gb;
                queryGitBranchForIntegratedCwd(m_terminalTabIntegratedWorkingDirectory[tabIndex], gb);
                m_terminalTabGitBranchFromCwd[tabIndex] = std::move(gb);
            }
    ...
    mgr->onError = [this, tabIndex](const std::string& error)
    {
        if (isShuttingDown())
            return;
        if (m_activeTerminalTab == static_cast<int>(tabIndex) && m_hwndPowerShellOutput)
    ...
    if (profile.shellPath.find("bash") != std::string::npos || profile.shellPath.find("wsl") != std::string::npos)
    {
        shellType = Win32TerminalManager::CommandPrompt;
    }

    mgr->start(shellType, cwdPtr);
...
{
    if (tabIndex < 0 || tabIndex >= (int)m_terminalTabs.size())
        return;
    if (m_terminalTabs.size() <= 1)
    ...
    if (tabIndex >= 0 && tabIndex < static_cast<int>(m_terminalTabCwdTelemetryCarry.size()))
    {
        m_terminalTabCwdTelemetryCarry.erase(m_terminalTabCwdTelemetryCarry.begin() + tabIndex);
        m_terminalTabIntegratedWorkingDirectory.erase(m_terminalTabIntegratedWorkingDirectory.begin() + tabIndex);
        if (tabIndex < static_cast<int>(m_terminalTabGitBranchFromCwd.size()))
            m_terminalTabGitBranchFromCwd.erase(m_terminalTabGitBranchFromCwd.begin() + tabIndex);
    }
...
void Win32IDE::showTerminalProfileMenu()
{
    ...
    {
        AppendMenuA(hMenu, MF_STRING, (UINT_PTR)(100 + i), m_terminalTabProfiles[i].name.c_str());
    }

    POINT pt;
    GetCursorPos(&pt);
    int cmd = TrackPopupMenuEx(hMenu, TPM_RETURNCMD | TPM_NONOTIFY, pt.x, pt.y, m_hwndMain, nullptr);
    DestroyMenu(hMenu);
...
LRESULT CALLBACK Win32IDE::TerminalTabBarProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        ...
        case WM_LBUTTONDOWN:
        {
            ...
            if (tabIndex < 0 || tabIndex >= (int)ide->m_terminalTabs.size())
                break;
...
void Win32IDE::initHoverTooltip()
{
    ...
    m_hoverFont =
        CreateFontA(-dpiScale(13), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                    CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FIXED_PITCH | FF_MODERN, "Consolas");
    m_hoverBoldFont =
        CreateFontA(-dpiScale(13), 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                    CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FIXED_PITCH | FF_MODERN, "Consolas");
    ensureTier2ClassesRegistered(GetModuleHandle(NULL));
...
void Win32IDE::shutdownHoverTooltip()
{
    ...
    if (m_hoverFont)
    {
        DeleteObject(m_hoverFont);
        m_hoverFont = nullptr;
    }
    if (m_hoverBoldFont)
    {
        DeleteObject(m_hoverBoldFont);
        m_hoverBoldFont = nullptr;
    }
}
...
void Win32IDE::showHoverTooltip(int screenX, int screenY, const std::string& content)
{
    ...
    if (popupW < dpiScale(200))
        popupW = dpiScale(200);
    if (popupW > dpiScale(600))
        popupW = dpiScale(600);
    if (popupH > dpiScale(400))
        popupH = dpiScale(400);
    ...
    if (screenX + popupW > mi.rcWork.right)
        screenX = mi.rcWork.right - popupW;
    if (screenY + popupH > mi.rcWork.bottom)
        screenY -= popupH + dpiScale(20);

    m_hwndHoverPopup =
        CreateWindowExA(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE, HOVER_CLASS, "",
                        WS_POPUP | WS_VISIBLE | WS_BORDER, screenX, screenY + dpiScale(20), popupW, popupH, m_hwndMain,
                        (HMENU)(UINT_PTR)IDC_HOVER_POPUP, GetModuleHandle(NULL), NULL);
...
void Win32IDE::onEditorMouseHover(int charPos)
{
    ...
    *(WORD*)lineText.data() = (WORD)lineLen;
    ...
    if (col < 0 || col >= (int)lineText.size())
        return;
...
LRESULT CALLBACK Win32IDE::HoverTooltipProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_PAINT:
        {
            ...
            if (!ide || ide->m_hoverContent.empty())
            {
                EndPaint(hwnd, &ps);
                return 0;
            }
...
void Win32IDE::shutdownSignatureHelp()
{
    ...
    if (m_signatureFont)
    {
        DeleteObject(m_signatureFont);
        m_signatureFont = nullptr;
    }
}
...
void Win32IDE::triggerSignatureHelp()
{
    ...
    *(WORD*)lineText.data() = (WORD)lineLen;
    ...
    while (nameStart >= 0 && (isalnum(lineText[nameStart]) || lineText[nameStart] == '_' || lineText[nameStart] == ':'))
        nameStart--;
    nameStart++;
...
void Win32IDE::showSignaturePopup(int screenX, int screenY)
{
    ...
    m_hwndSignaturePopup = CreateWindowExA(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE, SIGHELP_CLASS, "",
                                           WS_POPUP | WS_VISIBLE | WS_BORDER, screenX, screenY, popupW, popupH,
                                           m_hwndMain, (HMENU)(UINT_PTR)IDC_SIGHELP_POPUP, GetModuleHandle(NULL), NULL);
...
LRESULT CALLBACK Win32IDE::SignatureHelpProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
        case WM_PAINT:
        {
            ...
            if (!ide || ide->m_signatureContent.empty())
            {
                EndPaint(hwnd, &ps);
                return 0;
            }
            ...
            if (parenOpen != std::string::npos)
            {
                ...
                std::string params = sig.substr(
                    parenOpen + 1, (parenClose != std::string::npos ? parenClose - parenOpen - 1 : sig.size()));
...
void Win32IDE::shutdownReferencePanel()
{
    ...
    if (m_referenceFont)
    {
        DeleteObject(m_referenceFont);
        m_referenceFont = nullptr;
    }
}

void Win32IDE::showFindAllReferences(const std::string& symbol, const std::vector<ReferenceResult>& results)
{
    closeReferencePanel();

    if (results.empty())
    {
        appendToOutput("[References] No references found for '" + symbol + "'", "Output", OutputSeverity::Info);
        return;
    }
    ...
    m_hwndReferencePanel =
        CreateWindowExA(WS_EX_CLIENTEDGE, REFPANEL_CLASS, "", WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN, 0, panelY,
                        panelW, panelH, m_hwndMain, (HMENU)(UINT_PTR)IDC_REFPANEL, GetModuleHandle(NULL), NULL);
    ...
    // Close button
    CreateWindowExA(0, "BUTTON", "x", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, panelW - dpiScale(28), dpiScale(2),
                    dpiScale(24), dpiScale(22), m_hwndReferencePanel, (HMENU)(UINT_PTR)IDC_REFPANEL_CLOSE,
                    GetModuleHandle(NULL), NULL);

    // TreeView for results grouped by file
    m_hwndReferenceTree = CreateWindowExA(0, WC_TREEVIEWA, "",
                                          WS_CHILD | WS_VISIBLE | WS_VSCROLL | TVS_HASLINES | TVS_HASBUTTONS |
                                              TVS_LINESATROOT | TVS_SHOWSELALWAYS,
                                          0, headerH, panelW, panelH - headerH, m_hwndReferencePanel,
    ...
    for (const auto& [file, refs] : byFile)
    {
        ...
        {
            char lineLabel[512];
            snprintf(lineLabel, sizeof(lineLabel), "  L%d:%d  %s", ref->line + 1, ref->column,
                     ref->contextLine.c_str());
    ...
    LOG_INFO("[ReferencePanel] Showing " + std::to_string(results.size()) + " references for '" + symbol + "'");
}
...
{
    if (refIndex < 0 || refIndex >= (int)m_referenceResults.size())
        return;
...
void Win32IDE::initCodeLens()
{
    ...
    m_codeLensFont =
        CreateFontA(-dpiScale(11), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                    CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FIXED_PITCH | FF_MODERN, "Consolas");
...
void Win32IDE::shutdownCodeLens()
{
    ...
    if (m_codeLensFont)
    {
        DeleteObject(m_codeLensFont);
        m_codeLensFont = nullptr;
    }
}
#endif
...
void Win32IDE::refreshCodeLens()
{
    ...
    if (!m_codeLensEnabled || !m_hwndEditor)
        return;
    ...
    for (const auto& sym : m_outlineSymbols)
    {
        if (sym.kind == SK_Function || sym.kind == SK_Method || sym.kind == SK_Class || sym.kind == SK_Struct)
        {
            ...
            entry.text = std::to_string(entry.referenceCount) + " reference" + (entry.referenceCount != 1 ? "s" : "");
        ...
        for (const auto& child : sym.children)
        {
            if (child.kind == SK_Function || child.kind == SK_Method)
            {
                ...
                entry.text =
                    std::to_string(entry.referenceCount) + " reference" + (entry.referenceCount != 1 ? "s" : "");
...
int Win32IDE::countSymbolReferences(const std::string& symbol)
{
    ...
    while ((pos = content.find(symbol, pos)) != std::string::npos)
    {
        ...
        bool rightBound = (pos + symbol.size() >= content.size() ||
                           (!isalnum(content[pos + symbol.size()]) && content[pos + symbol.size()] != '_'));
...
{
    if (!m_codeLensEnabled || m_codeLensEntries.empty() || !m_hwndEditor)
        return;
    ...
    for (const auto& entry : m_codeLensEntries)
    {
        ...
        if (displayLine < 0 || displayLine >= visibleLines)
            continue;
...
void Win32IDE::toggleCodeLens()
{
    ...
    appendToOutput(std::string("[CodeLens] ") + (m_codeLensEnabled ? "Enabled" : "Disabled"), "Output",
                   OutputSeverity::Info);
}
...
void Win32IDE::initInlayHints()
{
    ...
    m_inlayHintFont =
        CreateFontA(-dpiScale(11), 0, 0, 0, FW_NORMAL, TRUE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                    CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FIXED_PITCH | FF_MODERN, "Consolas");
...
void Win32IDE::shutdownInlayHints()
{
    ...
    if (m_inlayHintFont)
    {
        DeleteObject(m_inlayHintFont);
        m_inlayHintFont = nullptr;
    }
}
#endif
...
void Win32IDE::refreshInlayHints()
{
    ...
    if (!m_inlayHintsEnabled || !m_hwndEditor)
        return;
    if (m_syntaxLanguage != SyntaxLanguage::Cpp && m_syntaxLanguage != SyntaxLanguage::JavaScript)
        return;
    ...
    while (std::getline(stream, line))
    {
        ...
        if (m_syntaxLanguage == SyntaxLanguage::Cpp)
        {
            ...
            if (trimmed.find("auto ") == 0 || trimmed.find("const auto ") == 0 || trimmed.find("auto& ") == 0 ||
                trimmed.find("const auto& ") == 0)
            {
        ...
        {
            if (trimmed.find("let ") == 0 || trimmed.find("const ") == 0 || trimmed.find("var ") == 0)
            {
...
std::string Win32IDE::inferTypeFromExpression(const std::string& expr)
{
    ...
    if (expr.find("std::vector") == 0)
        return "std::vector<...>";
    if (expr.find("std::map") == 0)
        return "std::map<...>";
    if (expr.find("std::set") == 0)
        return "std::set<...>";
    if (expr.find("std::unique_ptr") == 0)
        return "std::unique_ptr<...>";
    if (expr.find("std::shared_ptr") == 0)
        return "std::shared_ptr<...>";
    if (expr.find("std::make_unique") == 0)
        return "std::unique_ptr<...>";
    if (expr.find("std::make_shared") == 0)
        return "std::shared_ptr<...>";
    ...
    if (parenPos != std::string::npos)
    {
        ...
        // Common constructors
        if (funcName == "CreateFontA" || funcName == "CreateFontW" || funcName == "CreateFontIndirectA")
            return "HFONT";
        if (funcName == "CreateSolidBrush" || funcName == "CreateHatchBrush")
            return "HBRUSH";
        if (funcName == "CreatePen")
            return "HPEN";
        if (funcName == "CreateWindowExA" || funcName == "CreateWindowA")
            return "HWND";
        if (funcName == "GetDC")
            return "HDC";
        if (funcName == "LoadCursor")
            return "HCURSOR";
        if (funcName == "LoadIcon")
            return "HICON";

        // STL
        if (funcName == "std::to_string")
            return "std::string";
        if (funcName == "std::stoi")
            return "int";
        if (funcName == "std::stof")
            return "float";
        if (funcName == "std::stod")
            return "double";
    }
...
{
    if (!m_inlayHintsEnabled || m_inlayHintEntries.empty() || !m_hwndEditor)
        return;
...
void Win32IDE::toggleInlayHints()
{
    ...
    appendToOutput(std::string("[InlayHints] ") + (m_inlayHintsEnabled ? "Enabled" : "Disabled"), "Output",
                   OutputSeverity::Info);
}
...
bool Win32IDE::handleTier2Command(int commandId)
{
    switch (commandId)
    {
        case IDM_TIER2_GITDIFF:
            if (!m_currentFile.empty())
                showGitDiffSideBySide(m_currentFile);
            return true;
        ...
        case IDM_TIER2_OUTLINE_FILTER:
        {
            ...
            if (!m_outlineFilter.empty())
                MultiByteToWideChar(CP_UTF8, 0, m_outlineFilter.c_str(), -1, wFilter, 512);
            if (DialogBoxWithInput(L"Outline filter", L"Show symbols whose names contain (empty = all):", wFilter, 512))
            {
                char utf8[1024] = {};
                WideCharToMultiByte(CP_UTF8, 0, wFilter, -1, utf8, sizeof(utf8), nullptr, nullptr);
                filterOutlineView(std::string(utf8));
                appendToOutput(std::string("[Outline] Filter: ") + (utf8[0] ? utf8 : "(none)") + "\n", "Output",
                               OutputSeverity::Info);
            }
            return true;
        }
            ...
            if (m_hwndEditor)
            {
                ...
                if (lineLen > 0)
                {
                    ...
                    *(WORD*)lineText.data() = (WORD)lineLen;
                    ...
                    int col = sel.cpMin - lineStart;
                    int ws = col, we = col;
                    while (ws > 0 && (isalnum(lineText[ws - 1]) || lineText[ws - 1] == '_'))
                        ws--;
                    while (we < (int)lineText.size() && (isalnum(lineText[we]) || lineText[we] == '_'))
                        we++;
        ...
        {
            const std::string oldName = getWordAtCursor();
            if (oldName.empty())
            {
                appendToOutput("[RenamePreview] No identifier at caret.\n", "Output", OutputSeverity::Warning);
                return true;
            }
            wchar_t wNew[256] = {};
            MultiByteToWideChar(CP_UTF8, 0, oldName.c_str(), -1, wNew, 256);
            if (!DialogBoxWithInput(L"Rename preview", L"New name:", wNew, 256))
                return true;
            char newUtf8[512] = {};
            WideCharToMultiByte(CP_UTF8, 0, wNew, -1, newUtf8, sizeof(newUtf8), nullptr, nullptr);
            const std::string newName(newUtf8);
            if (newName.empty())
            {
                appendToOutput("[RenamePreview] Empty new name.\n", "Output", OutputSeverity::Warning);
                return true;
            }
            if (!m_hwndEditor)
                return true;
            const int n = GetWindowTextLengthA(m_hwndEditor) + 1;
            if (n <= 1)
                return true;
            std::string fileBuf((size_t)n, '\0');
            GetWindowTextA(m_hwndEditor, fileBuf.data(), n);
            fileBuf.resize((size_t)(n - 1));
            ...
            while (std::getline(iss, line))
            {
                ...
                {
                    const size_t hit = line.find(oldName, pos);
                    if (hit == std::string::npos)
                        break;
                    const bool leftOk = (hit == 0) || !isIdent(line[hit - 1]);
                    const bool rightOk = (hit + oldName.size() >= line.size()) || !isIdent(line[hit + oldName.size()]);
                    if (leftOk && rightOk)
                    {
                        RenameChange rc;
                        rc.filePath = m_currentFile.empty() ? std::string("(unsaved buffer)") : m_currentFile;
                        rc.line = lineNum;
                        rc.column = (int)hit;
                        rc.oldText = oldName;
                        rc.newText = newName;
                    ...
                    pos = hit + oldName.size();
                }
            }
            showRenamePreview(oldName, newName, changes);
            return true;
        }
            ...
            while (std::getline(stream, line))
            {
                ...
                while ((pos = line.find(symbol, pos)) != std::string::npos)
                {
                    ...
                    bool rightBound = (pos + symbol.size() >= line.size() ||
                                       (!isalnum(line[pos + symbol.size()]) && line[pos + symbol.size()] != '_'));
                    if (leftBound && rightBound)
                    {
                        ReferenceResult ref;
                        ...
                        if (ref.contextLine.size() > 100)
                            ref.contextLine.resize(100);
