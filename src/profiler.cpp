...
Profiler::Profiler()
    : m_isProfiling(false), m_totalAllocated(0), m_currentAllocated(0), m_peakAllocated(0), m_totalSamplesProcessed(0),
      m_totalTokensProcessed(0), m_lastGpuUsagePercent(0.0f), m_lastGpuMemoryMB(0.0f), m_cpuThresholdPercent(95.0f),
      m_memoryThresholdPercent(85.0f), m_gpuThresholdPercent(95.0f), m_advancedProfilingEnabled(false)
{
...
{
    const bool wasProfiling = m_isProfiling;
    m_isProfiling = false;
    if (wasProfiling && m_onProfilingCompleted)
    {
...
void Profiler::markPhaseEnd(const std::string& phaseName)
{
    ...
    const auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - it->second.startTime).count();
...
void Profiler::recordBatchCompleted(int sampleCount, int tokenCount)
{
    ...
    const auto batchDuration = std::chrono::duration_cast<std::chrono::milliseconds>(batchEnd - m_batchStart).count();
    m_batchLatencies.push_back(batchDuration);
    m_batchStart = batchEnd;

    if (m_advancedProfiler)
    {
        const size_t estimatedMemory = static_cast<size_t>(std::max(sampleCount, 0)) *
...
ProfileSnapshot Profiler::getCurrentSnapshot() const
{
    ...
    const auto elapsed =
        std::chrono::duration_cast<std::chrono::seconds>(std::chrono::high_resolution_clock::now() - m_profilingStart)
            .count();
    if (elapsed > 0)
    {
    ...
    if (!m_batchLatencies.empty())
    {
        snapshot.batchLatencyMs = static_cast<float>(
            std::accumulate(m_batchLatencies.begin(), m_batchLatencies.end(), 0.0) / m_batchLatencies.size());
    }
...
std::string Profiler::getProfilingReport() const
{
    ...
    if (!m_phases.empty())
    {
        ...
        for (const auto& [phaseName, phaseData] : m_phases)
        {
            ...
            {
                const double avgDuration = static_cast<double>(phaseData.totalMs) / phaseData.durations.size();
                ss << "  " << phaseName << ": " << avgDuration << " ms (avg), " << phaseData.durations.size()
                   << " calls\n";
            }
        }
    }
...
float Profiler::getMemoryUsageMB() const
{
    ...
    {
        return static_cast<float>((memInfo.ullTotalPhys - memInfo.ullAvailPhys) / (1024.0 * 1024.0));
    }
#endif
...
void Profiler::analyzeMetrics()
{
    ...
    {
        m_onPerformanceWarning("High CPU usage detected: " + std::to_string(snapshot.cpuUsagePercent) + "%");
    }

    if (snapshot.memoryUsageMB > (m_memoryThresholdPercent / 100.0f) * 8192.0f && m_onPerformanceWarning)
    {
        m_onPerformanceWarning("High memory usage detected: " + std::to_string(snapshot.memoryUsageMB) + " MB");
    }

    if (snapshot.gpuUsagePercent > m_gpuThresholdPercent && m_onPerformanceWarning)
    {
        m_onPerformanceWarning("High GPU usage detected: " + std::to_string(snapshot.gpuUsagePercent) + "%");
    }
}
...
void Profiler::enableAdvancedProfiling()
{
    ...
    if (m_advancedProfiler->initialize())
    {
        ...
        RawrXD::Logging::Logger::instance().info("Advanced performance profiling enabled", "Profiler");
    }
    else
    {
        m_advancedProfiler.reset();
        RawrXD::Logging::Logger::instance().error("Failed to initialize advanced performance profiler", "Profiler");
    }
}

void Profiler::disableAdvancedProfiling()
{
    // Always finish cleanly if an instance exists (flag can be wrong after partial failure paths).
    if (m_advancedProfiler)
    {
...
std::vector<std::string> Profiler::detectBottlenecks()
{
    ...
    if (m_advancedProfiler)
    {
        ...
        {
            std::stringstream ss;
            ss << "Bottleneck: " << bottleneck.componentName << " (" << bottleneck.bottleneckType
               << ") - Severity: " << bottleneck.severityScore << " - " << bottleneck.description;
...
void Profiler::monitorResourceContention(const std::string& resourceName, const std::string& resourceType)
{
    if (m_advancedProfiler)
    {
