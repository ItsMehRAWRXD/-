...
rmsnorm_avx2_sq_loop:
    vmovups ymm1, YMMWORD PTR [rsi + rax*4]     ; Load 8 floats from x
...
rmsnorm_avx2_norm_loop:
    vmovups ymm1, YMMWORD PTR [rsi + rax*4]     ; x[i..i+7]
    vmovups ymm2, YMMWORD PTR [rdi + rax*4]     ; weight[i..i+7]
    vmulps ymm1, ymm1, ymm4        ; x * scale
    vmulps ymm1, ymm1, ymm2        ; * weight
    vmovups YMMWORD PTR [rbx + rax*4], ymm1     ; store y
    add eax, 8
    dec ecx
...
rmsnorm_512_tail:
    ...
    kmovw k1, edx
    vmovups zmm1{k1}{z}, ZMMWORD PTR [rsi + rax*4]
...
rmsnorm_512_norm_loop:
    vmovups zmm1, ZMMWORD PTR [rsi + rax*4]
    vmovups zmm2, ZMMWORD PTR [rdi + rax*4]
    vmulps zmm1, zmm1, zmm4
    vmulps zmm1, zmm1, zmm2
    vmovups ZMMWORD PTR [rbx + rax*4], zmm1
    add eax, 16
    dec ecx
...
rmsnorm_512_norm_tail:
    ...
    kmovw k1, edx
    vmovups zmm1{k1}{z}, ZMMWORD PTR [rsi + rax*4]
    vmovups zmm2{k1}{z}, ZMMWORD PTR [rdi + rax*4]
    vmulps zmm1, zmm1, zmm4
    vmulps zmm1, zmm1, zmm2
    vmovups ZMMWORD PTR [rbx + rax*4]{k1}, zmm1
...
softmax_512_sub_loop:
    ...
    vmovups ZMMWORD PTR [rsi + rax*4], zmm1
    add eax, 16
    dec ecx
...
vdot_avx2_loop:
    vmovups ymm1, YMMWORD PTR [rcx + rax*4]     ; a[i..i+7]
    vmovups ymm2, YMMWORD PTR [rdx + rax*4]     ; b[i..i+7]
...
; =============================================================================
; Note: RoPE requires cos/sin computation which is complex in pure ASM.
; This kernel handles the rotation multiply step; cos/sin values are
; precomputed by the C++ caller and passed in the q/k buffers interleaved.
; For the full RoPE, the C++ fallback is used. This is a placeholder for
...
; =============================================================================
; Note: Full fused MLP in pure ASM is complex. This provides the GeLU
; activation phase using AVX2. The full pipeline uses C++ + this kernel.
native_fused_mlp_avx2 PROC FRAME
    ...
    ; Placeholder: delegates to C++ fallback for full pipeline
    ; The GeLU AVX2 micro-kernel can be called separately
