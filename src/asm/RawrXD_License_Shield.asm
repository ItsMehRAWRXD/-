...
IsDebuggerPresent_Native PROC
    ...
    mov     rax, gs:[60h]
    movzx   eax, byte ptr [rax + 2]        ; PEB.BeingDebugged (offset 02h)
    ret
...
@@hash_done:
    ...
    lea     rax, g_ExpectedTextHash
    cmp     qword ptr [rax], 0
    jne     @@compare

    ; First-run: record baseline hash
    mov     qword ptr [rax], rbx
    lea     rax, g_TextHashValid
    mov     byte ptr [rax], 1
...
; When a debugger attaches, the heap manager sets HEAP_GROWABLE (002h) in
...
Shield_CheckKernelDebug PROC FRAME
    ...
    ;   SystemKernelDebuggerInformation (023h),
    ;   &debugInfo,
    ;   sizeof(debugInfo),   ; 2 bytes needed, give 8 for alignment
    ;   &returnLength
    ; )
    sub     rsp, 28h                        ; Shadow space
    mov     ecx, SystemKernelDebuggerInformation  ; Class = 023h
    lea     rdx, debugInfo                  ; Output buffer
    mov     r8d, 8                          ; Buffer m_size
...
Shield_ApplyFingerprint PROC FRAME
    ...
    mov     rcx, rdx        ; RCX = m_size (loop counter)
...
@@sid_evaluate:
    ...
    ; All 5 layers must pass (bitmask = 01Fh)
    cmp     ebx, 01Fh
    je      @@sid_clean
