...
EXTERNDEF NanoDisk_GetJobStatus:PROC
EXTERNDEF NanoDisk_GetJobResult:PROC
EXTERNDEF NanoDisk_AbortJob:PROC
...
AGENT_MODE_QUANTIZE      equ 19     ; NanoDisk: -quantize model.gguf rank
...
.data
    ...
    szModeQuantize       db "Quantize (NanoDisk Bridge)", 13, 10
                         db "============================================", 13, 10, 0
    szCLIQuantStart      db "[*] Mode 19: Background model quantization via NanoDisk Bridge", 13, 10, 0
    szCLIQuantPath       db "[*] Source GGUF: ", 0
    szCLIQuantRank       db "[*] Target rank: ", 0
    szCLIQuantQueued     db "[+] Quantize job queued. Waiting for completion...", 13, 10, 0
    szCLIQuantDone       db "[+] Quantization complete!", 13, 10, 0
    szCLIQuantFailed     db "[-] Quantization failed.", 13, 10, 0
    szCLIQuantRatio      db "[+] Compression ratio: ", 0
    szCLIQuantMatrix     db "[+] Output matrix at: 0x", 0
...
atl_quantize:
    ; Mode 19: NanoDisk async quantize
    ; UserData = source GGUF path ptr, CallbackPtr = rank (packed as QWORD)
    call DiskKernel_Init
    call NanoDisk_Init

    ; Extract path from UserData, rank from CallbackPtr
    mov  rcx, g_AgentCtx.UserData       ; Source GGUF path
    mov  edx, dword ptr g_AgentCtx.CallbackPtr ; Target rank
    call AgentTool_QuantizeModel

    test rax, rax
    jz   atl_quantize_fail

    mov  g_AgentCtx.Status, AGENT_STATUS_RECOVERING  ; Reuse "busy" status
    lea  rax, g_AgentCtx
    jmp  atl_exit
...
atc_skip_thread:
    ...
    ; Shutdown NanoDisk Bridge (safe to call even if not initialized)
    call NanoDisk_Shutdown
