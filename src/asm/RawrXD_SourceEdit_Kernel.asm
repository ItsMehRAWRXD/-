...
@@append_old:
    ...
    ; ======== PRODUCTION: Strip read-only flag from original ========
    ; SetFileAttributesW(originalPath, FILE_ATTRIBUTE_NORMAL)
    ; Temporarily remove read-only so rename succeeds in restricted envs.
    mov     rcx, r12                    ; lpFileName = originalPath
...
@@skip_attr_strip:
    ...
    ; ======== PRODUCTION: FlushFileBuffers on temp before rename ========
    ; Re-open temp to flush, ensuring content is physically on disk
    ; before we commit the rename transaction.
    lea     rcx, [rbp + TMP_PATH_OFF]   ; tmpPath
    mov     edx, GENERIC_WRITE          ; need write access for flush
    xor     r8d, r8d
    xor     r9, r9
...
@@skip_flush:
    ...
    ; ======== STEP 4: Safe-Rename via ReplaceFileW ========
    ; ReplaceFileW preserves metadata and security descriptors.
    ; Fallback: MoveFileEx chain if ReplaceFileW fails.
    ; ReplaceFileW(originalPath, tmpPath, backupPath, 0, NULL, NULL)
    mov     rcx, r12                    ; lpReplacedFileName = originalPath
    lea     rdx, [rbp + TMP_PATH_OFF]   ; lpReplacementFileName = tmpPath
    xor     r8, r8                      ; lpBackupFileName = NULL (backup already done)
    xor     r9d, r9d                    ; dwReplaceFlags = 0
    mov     QWORD PTR [rsp + 32], 0     ; lpExclude = NULL
