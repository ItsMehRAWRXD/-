...
szCheckingAI            BYTE "Checking local inference engine...", 0
szAIOK                  BYTE "Sovereign inference ready.", 13, 10, 0
szAIOffline             BYTE "No model loaded. Use /load <model.gguf>", 13, 10, 0
szModelPath             BYTE "models\default.gguf", 0
...
; ----------------------------------------------------------------------------
; CmdVerifyAI - Check if sovereign inference engine is ready
; No network dependency - checks local model status
; ----------------------------------------------------------------------------
CmdVerifyAI PROC
    push rbx
    sub rsp, 48
    ...
    ; Check if SovereignStandaloneEngine is loaded
    ; In pure MASM build, we check for model file existence
    lea rcx, szModelPath
    call FileExists
    test rax, rax
    jz @@offline
